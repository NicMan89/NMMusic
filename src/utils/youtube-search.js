...
// ⚠️ NON committare la tua API key reale su GitHub!
...
export class YouTubeSearchService {
  ...
  async search(query, maxResults = 20) {
    if (!this.apiKey || this.apiKey === 'YOUR_YOUTUBE_API_KEY_HERE') {
      console.warn('⚠️ YouTube API key non configurata');
  ...
  mockSearchResults(query) {
    console.log('📝 Using mock search results (API key not configured)');
    ...
    return [
      {
        ...
        description: 'Questo è un risultato di esempio. Configura la YouTube API key per risultati reali.'
      }
    ];
  }
